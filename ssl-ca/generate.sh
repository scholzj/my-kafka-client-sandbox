...
# Generate server keys - ext
cfssl gencert -ca keys/ca.pem -ca-key keys/ca-key.pem server-ext-0.json | cfssljson -bare keys/server-ext-0
cfssl gencert -ca keys/ca.pem -ca-key keys/ca-key.pem server-ext-1.json | cfssljson -bare keys/server-ext-1
cfssl gencert -ca keys/ca.pem -ca-key keys/ca-key.pem server-ext-2.json | cfssljson -bare keys/server-ext-2
...
# Generate full chain server CRTs - ext
cat keys/server-ext-0.pem keys/ca.pem > keys/server-ext-0-full-chain.pem
cat keys/server-ext-1.pem keys/ca.pem > keys/server-ext-1-full-chain.pem
cat keys/server-ext-2.pem keys/ca.pem > keys/server-ext-2-full-chain.pem
...
openssl pkcs12 -export -out keys/server-ext-0.p12 -in keys/server-ext-0-full-chain.pem -inkey keys/server-ext-0-key.pem -password pass:$PASSWORD
openssl pkcs12 -export -out keys/server-ext-1.p12 -in keys/server-ext-1-full-chain.pem -inkey keys/server-ext-1-key.pem -password pass:$PASSWORD
...
keytool -importkeystore -srckeystore keys/server-ext-0.p12 -srcstoretype PKCS12 -srcstorepass $PASSWORD -destkeystore keys/server-ext-0.keystore -deststoretype JKS -deststorepass $PASSWORD -noprompt
keytool -importkeystore -srckeystore keys/server-ext-1.p12 -srcstoretype PKCS12 -srcstorepass $PASSWORD -destkeystore keys/server-ext-1.keystore -deststoretype JKS -deststorepass $PASSWORD -noprompt
