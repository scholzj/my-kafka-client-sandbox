...
public class MessageConsumerCompaction {
    ...
    public static void main(String[] args)
    {
        ...
        consumer.subscribe(Collections.singletonList("myCompactedTopic"));
        ...
        for (TopicPartition partition : consumer.assignment()) {
            System.out.println("-I- Partition " + partition + " starts at " + consumer.position(partition));
        }*/
        ...
        final Map<String, Integer> compactedStats = new HashMap<String, Integer>();
        ...
        while (true)
        {
            ...
            {
                if (compactedStats.containsKey(record.key()))
                {
                    compactedStats.put(record.key(), compactedStats.get(record.key()) + 1);
                }
                else
                {
                    compactedStats.put(record.key(), 1);
                }
        ...
        System.out.println("-I- ####################################");
        System.out.println("-I- Key   | Counter");

        Integer sum = 0;

        for (String k : compactedStats.keySet())
        {
            System.out.println("-I- " + k + " | " + compactedStats.get(k));
            sum += compactedStats.get(k);
        }
